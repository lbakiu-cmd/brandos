...
= 1.6.9 =
* Added a "⚡ Auto-Fix All" button to the Action Plan tab -- rewrites the post's content via the AIVisibility SEO platform to address every open high/medium-priority check (headings, lists, direct-answer blockquote, keyword placement, etc.), then applies the update and refreshes the scores. Statistics/data-point requests are handled honestly: real named authorities may be cited for general facts, but no numbers are invented for this specific business.

= 1.6.8 =
* Fixed the publish-post endpoint dropping the "schemas" param entirely -- FAQPage and LocalBusiness JSON-LD generated by the platform was never actually saved to the post, so every published article scored 0 schemas in the GEO analyzer's multi-schema check regardless of what was sent.
