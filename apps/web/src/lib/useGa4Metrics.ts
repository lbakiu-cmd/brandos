...
export function useGa4Metrics(timeRange: TimeRangeKey) {
  ...
  useEffect(() => {
    ...
    apiFetch<any>(`/integrations/google/analytics?days=${days}`)
      .then(data => {
        if (data?.measurementVersion !== 2) throw new Error("Unverified analytics response");
        if (!cancelled) setResult({ range: timeRange, data, error: null });
      })
      .catch(() => {
        if (!cancelled) setResult({ range: timeRange, data: null, error: "Google Analytics data is unavailable for this period." });
      });
