...
export function TimeRangeFilter({
  ...
  if (variant === "segmented") {
    return (
      <div className="flex items-center rounded-xl bg-slate-900/90 border border-slate-800 p-1 text-xs font-semibold shadow-inner">
        {showIcon && <Calendar className="h-3.5 w-3.5 text-slate-400 ml-1.5 mr-1" />}
  ...
  return (
    <div className="relative inline-block text-left" ref={dropdownRef}>
      <button
        ...
        className={`flex items-center gap-1.5 rounded-lg border border-slate-800 bg-slate-950/70 px-2.5 py-1 text-[11px] font-semibold text-slate-300 transition hover:bg-slate-800/80 hover:text-white hover:border-slate-700 shadow-sm ${
      ...
      {isOpen && (
        <div className="absolute right-0 z-50 mt-1.5 w-44 origin-top-right rounded-xl border border-slate-800 bg-slate-900/95 p-1.5 shadow-2xl backdrop-blur-xl animate-in fade-in zoom-in-95 duration-150">
          <div className="px-2 py-1 text-[10px] font-bold uppercase tracking-wider text-slate-500 border-b border-slate-800/80 mb-1">
            Filter Time Window
          </div>
