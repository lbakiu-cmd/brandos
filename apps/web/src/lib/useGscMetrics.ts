...
export function useGscMetrics(timeRange: TimeRangeKey) {
  ...
  useEffect(() => {
    ...
    apiFetch<any>(`/integrations/google/search-analytics?days=${days}`)
      .then(data => {
      ...
      .catch(() => {
        if (!cancelled) setResult({ range: timeRange, data: null, error: "Google Search Console data is unavailable for this period." });
      });
